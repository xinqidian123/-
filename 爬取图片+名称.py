...
def get_content(index: int, ans: list):
    ...
    # img_url = img_pattern.search(img_src).group(1)
...
if __name__ == "__main__":
    n = int(input("输入需要的爬虫数（1-100）："))
    Group_buying_URL = create_urls()  # 获取星巴克、库迪、瑞幸三家所有团购信息所在页面的url
    num = min(n, len(Group_buying_URL))  # for循环应当遍历的次数
    if n > num:
        print("您当前输入数字太大！")
    for i in range(0, num):
        ...
        print(
            f"当前进度{round(100 * (i + 1) / num, 2)}%,还剩{min(n, len(Group_buying_URL)) - i - 1}个"
        )
